          ...
          <div class="terms-scroll overflow-y-auto px-6 py-5 text-[14px] sm:text-[15px] leading-[26px] sm:leading-[28px] text-[#2D4745] dark:text-dark-text/80">
            ...
            <div v-else-if="termsSections.length">
              ...
              >
                ...
                >
                  {{ localizeCategory(section.category) }}
                </h3>
                ...
                >
                  <p class="font-bold text-[#8B6F2E] dark:text-[#ECD0A0] mb-1.5">
                    {{ pickLocalized(item, 'title', 'title_en') }}
                  </p>
                  <p class="text-[#5B6B76] dark:text-[#B7C4C6]">
                    {{ pickLocalized(item, 'description', 'description_en') }}
                  </p>
                </div>
              </div>
            </div>
...
// ==================== انتخاب مقدار لوکالایز شده ====================
// بک‌اند برای آیتم‌های قوانین فیلدهای title_en / description_en رو
// جداگانه و بدون توجه به هدر X-Language برمی‌گردونه، پس انتخاب
// باید دستی توی فرانت انجام بشه (همون الگوی faq.vue / resume).
const pickLocalized = (obj, faKey, enKey) => {
  const enVal = obj?.[enKey];
  return (locale.value === 'en' && enVal) ? enVal : obj?.[faKey];
};

// ==================== ترجمه‌ی اسم دسته‌بندی‌ها ====================
// نکته‌ی مهم: خودِ کلید دسته‌بندی (مثلاً "قوانین و مقررات نادرتک") از
// بک‌اند به شکل کلید آبجکت میاد و هیچ نسخه‌ی _en برای دسته‌بندی وجود
// نداره (فقط آیتم‌های داخلش title_en/description_en دارن). بنابراین
// باید یه نگاشت استاتیک فارسی -> انگلیسی اینجا نگه داریم و هر بار که
// دسته‌بندی جدیدی توی بک‌اند اضافه شد، این نگاشت هم به‌روزرسانی بشه.
const categoryTranslations = {
  'قوانین و مقررات نادرتک': 'NaderTech Terms and Regulations',
  'قوانین خرید بلیت ایونت': 'Event Ticket Purchase Rules',
  'قوانین خدمات طراحی سایت و تولید محتوا': 'Website Design & Content Production Service Rules',
  'قوانین درخواست همکاری': 'Collaboration Request Rules',
  'پرداخت': 'Payment',
  'مسئولیت کاربران': 'User Responsibilities',
  // دسته‌ی بدون عنوان (کلید خالی "" در پاسخ API) — حریم خصوصی/تماس با ما
  '': '',
};

const localizeCategory = (category) => {
  if (locale.value !== 'en') return category;
  // اگه نگاشت رو داشتیم همون رو برگردون، وگرنه فارسی رو به‌عنوان fallback نشون بده
  return category in categoryTranslations ? categoryTranslations[category] : category;
};

// ==================== دریافت قوانین از بک‌اند ====================
const loading = ref(false);
const error = ref(false);
const termsSections = ref([]); // آرایه‌ای از { category, items: [{ id, title, title_en, description, description_en }] }
