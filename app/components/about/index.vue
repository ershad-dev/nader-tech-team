<template>
<div class="max-w-6xl 2xl:max-w-[1600px] mx-auto py-16 px-5 md:px-10 lg:px-14 xl:px-16 2xl:px-12 overflow-x-hidden" :dir="isRtl ? 'rtl' : 'ltr'">    <!-- Hero Section -->
...
// =====================================================
// اتصال به API صفحه about + دوزبانه‌سازی
// -----------------------------------------------------
// بک‌اند برای هر کلید متنی/تصویری، نسخه‌ی انگلیسی رو به‌صورت
// یک ردیف جدا با کلید "${key}_en" برمی‌گردونه (مثلاً title و title_en).
// چون هر دو نسخه توی همون یک درخواست میان، آرایه‌ی خام رو نگه می‌داریم
// و pageData رو به‌صورت computed می‌سازیم؛ با تغییر locale، بدون فچ
// مجدد از سرور، خودش دوباره محاسبه میشه.
// اگه نسخه‌ی en برای یک کلید هنوز توسط ادمین پر نشده باشه (خالی/نال)،
// fallback به نسخه‌ی فارسی همون کلید انجام میشه.
// =====================================================
...
// کلیدهای متنی/تصویری صفحه‌ی about که باید نمایش داده بشن
...
const pageData = computed(() => {
  ...
  pageKeys.forEach((key) => {
    ...
    // فقط وقتی زبان انگلیسیه و نسخه‌ی en واقعاً پر شده ازش استفاده کن؛
    // در غیر این صورت (زبان fa، یا en ولی هنوز ترجمه نشده) از نسخه‌ی fa استفاده کن
...
// =====================================================
// بخش خدمات - اتصال به API project-services (ساختار درختی) + دوزبانه‌سازی
// -----------------------------------------------------
// اینجا برخلاف pageData، فیلدهای انگلیسی مستقیم روی همون آبجکت
// هستن (title_en، description_en) نه به‌شکل ردیف جدا، چون این
// مدل اسکیمای ثابت دیتابیسی داره.
// =====================================================
...
// انتخاب فیلد en در صورت وجود و پر بودن، وگرنه fallback به fa
