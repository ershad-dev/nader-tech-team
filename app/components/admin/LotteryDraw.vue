...
const drawnWinners = ref([])
const entriesPool = ref([])
...
const startDraw = async () => {
  ...
  if (!entriesPool.value.length) {
    errorMsg.value = 'شرکت‌کننده‌ای برای قرعه‌کشی وجود ندارد.'
    return
  }

  isDrawing.value = true

  // انتخاب رندوم به تعداد winner_count از بین شرکت‌کنندگان
  const shuffled = [...entriesPool.value].sort(() => Math.random() - 0.5)
  const selected = shuffled.slice(0, winnerCount.value)
  const winnersPayload = selected.map((entry, index) => ({
    user_id: entry.user_id,
    position: index + 1,
  }))

  try {
    const res = await $fetch(`${API_BASE}/admin/lotteries/${props.lotteryId}/draw`, {
      method: 'POST',
      headers: authHeader(),
      body: { winners: winnersPayload },
    })
    drawnWinners.value = res.data || []
    showResults.value = true
  } catch (err) {
    console.error(err)
    if (err?.response?.status === 409) errorMsg.value = 'این قرعه‌کشی قبلاً انجام شده است.'
    else if (err?.response?.status === 422) errorMsg.value = 'خطای اعتبارسنجی در ارسال برندگان.'
    else errorMsg.value = 'خطا در انجام قرعه‌کشی.'
  } finally {
...
const showPreviousResults = async () => {
  try {
    const res = await $fetch(`${API_BASE}/admin/lotteries/${props.lotteryId}/winners`, {
      headers: authHeader(),
    })
    drawnWinners.value = res.data || []
    showResults.value = true
    ...
    console.error(err)
    errorMsg.value = 'خطا در دریافت نتایج.'
  }
}
...
<template>
  ...
  <div
    ...
    dir="rtl"
  >
    <div v-if="errorMsg" class="text-center text-red-300 font-bold mb-4">{{ errorMsg }}</div>
    ...
    >
      ...
      >
        <span class="absolute inset-0 flex items-center justify-center text-white font-bold text-[16px] sm:text-[22px] lg:text-[36px] leading-none">
          {{ i }}
        </span>
      </div>
    </div>

    <LotteryResults v-else :winners="drawnWinners" />

    <div v-if="!showResults" class="flex flex-col items-center gap-3 sm:gap-4 min-[1920px]:gap-6 px-2 sm:px-0">
      <button
        @click="startDraw"
        :disabled="isDrawing"
        class="w-full max-w-[478px] sm:w-[478px] bg-white text-[#3949ab] px-6 sm:px-10 py-3 rounded-full font-bold shadow-xl flex items-center justify-center gap-2 h-[44px] text-sm sm:text-base disabled:opacity-60"
      >
