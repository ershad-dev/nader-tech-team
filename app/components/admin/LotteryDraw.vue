<template>
  <div class="lottery-scroll min-h-screen bg-[url('/images/bg-lottery.png')] bg-cover bg-center bg-no-repeat bg-fixed text-white flex flex-col  p-8 transition-colors duration-700" dir="rtl">
    
    <div v-if="!showResults" class="grid grid-cols-[repeat(6,max-content)] justify-center gap-4 mb-10" dir="ltr">
<div
  v-for="i in 36"
  :key="i"
  class="circle-cut w-[126px] h-[106px] rounded-[20px] bg-gradient-to-br from-white/35 via-[#6EA8FF]/25 to-[#1E40AF]/70 backdrop-blur-xl border border-white/30 shadow-[0_8px_32px_rgba(30,64,175,0.35)] relative"
>
  <span
    class="absolute inset-0 flex items-center justify-center text-white font-bold text-[36px] leading-none"
  >
    {{ i }}
  </span>
</div>
    ...
    <div v-if="!showResults" class="flex flex-col items-center gap-4">
      <button class="w-[478px] bg-white text-[#3949ab] px-10 py-3 rounded-full font-bold shadow-xl flex items-center justify-center gap-2 h-[44px]">
...
<rect width="32" height="32" fill="white" transform="matrix(0 1 -1 0 32 0)"/>
      ...
      <button 
        ...
        class="w-[283px] h-[44px] bg-[#7e57c2] text-white px-10 py-3 rounded-full font-bold shadow-xl flex items-center justify-center gap-2"
      >
