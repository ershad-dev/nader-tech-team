<template>
  <div class="max-w-[1100px] mx-auto py-8 sm:py-12 md:py-16 px-4 sm:px-5 md:px-6 bg-[#ABD7D8]/25 rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[3rem] mt-6 sm:mt-8 md:mt-10 mb-6 sm:mb-8 md:mb-10 shadow-xl" dir="rtl">

    <!-- لودینگ -->
    <div v-if="pending" class="text-center py-16 text-slate-500">در حال بارگذاری مقاله...</div>

    <!-- خطا / 404 -->
    <div v-else-if="error" class="text-center py-16">
      <p class="text-red-500 text-lg mb-2">مقاله پیدا نشد</p>
      <NuxtLink to="/articles" class="text-[#2D7A6F] underline">بازگشت به مقالات</NuxtLink>
    </div>

    <template v-else-if="article">
      <h1 class="text-[20px] sm:text-[26px] md:text-[32px] font-bold text-center mb-5 sm:mb-6 md:mb-8 leading-snug">
        {{ article.title }}
      </h1>
      ...
      <div class="flex justify-center items-center gap-4 mb-6 sm:mb-8">
        ...
        <span class="text-slate-400 text-xs">{{ article.views_count }} بازدید</span>
      </div>

      <!-- content از سرور HTML است -->
      <div
        class="prose prose-lg max-w-none space-y-4 sm:space-y-5 md:space-y-6 text-justify leading-[26px] sm:leading-[36px] md:leading-[50px] text-[14px] sm:text-[17px] md:text-[20px] text-[#0F184B] font-noto-regular"
